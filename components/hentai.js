...
module.exports.run = async (Papi, Discord, message, args, fulltext, con, ops) => {
	...
	booru.search(`${source}`, [`${fulltext}`], { limit: 1, random: true }).then(booru.commonfy).then(async images => {
		if (source === 'lolibooru.moe') {
			let newfileurl = await images[0].common.file_url.replace(/ /g, '%20');
			const HentaiIMG = new Discord.RichEmbed()
				.setColor(process.env.COLOR_H)
				.setFooter(`${message.author.username} | Filter: ${fulltext}`, message.author.displayAvatarURL)
				.setImage(newfileurl);
			msg.delete();
